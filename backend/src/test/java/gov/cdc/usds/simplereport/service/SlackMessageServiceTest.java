...
public class SlackMessageServiceTest {
  ...
  public void sendMessage_noDividers_formatOk() {
    ...
    ArgumentCaptor<RequestEntity<SlackMessageService.SlackMessage>> captor =
    ...
    DataHubConfig config =
        new DataHubConfig(false, null, 0, null, "https://hooks.slack.com/fake", null, null);
    ...
    assertEquals(
        List.of(SlackBlockType.header, SlackBlockType.section, SlackBlockType.section),
    ...
    assertEquals(
        List.of("Hello, test harness", "Humpty", "Dumpty"),
        message.getBlocks().stream().map(this::getText).collect(Collectors.toList()));
    assertEquals(
        List.of(SlackBlockType.plain_text, SlackBlockType.mrkdwn, SlackBlockType.mrkdwn),
