...
@Service
public class TestResultFhirConversionService implements FhirConversionStrategy {
    ...
    @Override
    public Bundle convertRowToFhirBundle(FileRow row, UUID orgId) {
        TestResultRow testResultRow = (TestResultRow) row;
        return convertTestResultRowToFhir(testResultRow, orgId);
    }


    private Bundle convertTestResultRowToFhir(TestResultRow row, UUID orgId) {
        ...
        var device = bulkTestResultUploadFhirConverter.convertToDevice(manufacturer, modelName, deviceId.toString());
        ...
        var aoeObservations =
                bulkTestResultUploadFhirConverter.convertToAOEObservation(testEventId, symptomatic, symptomOnsetDate);
