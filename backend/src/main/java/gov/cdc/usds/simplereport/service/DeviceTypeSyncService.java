...
public class DeviceTypeSyncService {
  ...
  public boolean hasUpdates(
      ...
      DeviceType existing) {
    if (existing.getSwabTypes() == null || existing.getSwabTypes().isEmpty()) {
      return true;
    }

    if (existing.getSupportedDiseaseTestPerformed() == null
        || existing.getSupportedDiseaseTestPerformed().isEmpty()) {
      return true;
    }
    ...
    boolean hasSwabUpdates =
        !incomingSwabs.stream()
                .allMatch(d -> existing.getSwabTypes().stream().anyMatch(b -> b.equals(d)))
