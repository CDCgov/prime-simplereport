...
public class DeviceTypeSyncService {
  ...
  @Transactional
  public DeviceType updateDeviceType(UpdateDeviceType deviceToSync) {
    DeviceType device = _deviceTypeRepo.findById(deviceToSync.getInternalId()).get();
    if (deviceToSync.getName() != null) {
      device.setName(deviceToSync.getName());
    }
    if (deviceToSync.getManufacturer() != null) {
      device.setManufacturer(deviceToSync.getManufacturer());
    }
    if (deviceToSync.getModel() != null) {
      device.setModel(deviceToSync.getModel());
    }
    if (deviceToSync.getTestLength() > 0) {
      device.setTestLength(deviceToSync.getTestLength());
    }

    if (deviceToSync.getSwabTypes() != null) {
    ...
    if (deviceToSync.getSupportedDiseaseTestPerformed() != null) {
      List<DeviceTypeDisease> deviceTypeDiseaseList =
          createUpdatedDeviceTypeDiseaseList(
              deviceToSync.getSupportedDiseaseTestPerformed(), device);
  ...
  public boolean hasUpdates(
      List<DeviceTypeDisease> incomingDiseases, List<UUID> incomingSwabs, DeviceType existing) {
    if (existing.getSwabTypes() == null
        || existing.getSwabTypes().isEmpty() && !incomingSwabs.isEmpty()) {
      return true;
    }

    if (existing.getSupportedDiseaseTestPerformed() == null
        || existing.getSupportedDiseaseTestPerformed().isEmpty() && !incomingDiseases.isEmpty()) {
      return true;
    }
                ...
                .allMatch(
                    d ->
                        existing.getSupportedDiseaseTestPerformed().stream()
                            .anyMatch(b -> areEqualIgnoreDeviceTypeId(b, d)))
            || incomingDiseases.size() != existing.getSupportedDiseaseTestPerformed().size();

    boolean hasSwabUpdates =
        CollectionUtils.isEqualCollection(
            incomingSwabs,
            existing.getSwabTypes().stream().map(IdentifiedEntity::getInternalId).toList());
  ...
  @Transactional
  public void updateDeviceTypesAsync(HashMap<DeviceType, DeviceType> devicesToUpdate) {
    devicesToUpdate.forEach(
        (existing, incoming) -> {
          List<UUID> swabTypeUUIDs = createUpdateOrFetchSpecimenTypes(incoming);
          DeviceType updatedDevice =
              updateDeviceType(convertToUpdateDeviceType(existing, incoming, swabTypeUUIDs));
          String msg =
              String.format(
  ...
  @Async("applicationTaskExecutor")
  @Transactional
  public void createDeviceTypesAsync(List<DeviceType> devicesToCreate) {
