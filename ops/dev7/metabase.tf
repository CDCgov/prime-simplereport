...
module "metabase_service" {
  ...
  ai_instrumentation_key = data.terraform_remote_state.persistent_dev7.outputs.app_insights_instrumentation_key
  key_vault_id           = data.azurerm_key_vault.sr_global.id
  tenant_id              = data.azurerm_client_config.current.tenant_id

  postgres_server_name = data.terraform_remote_state.persistent_dev7.outputs.postgres_server_name
  postgres_url         = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.metabase_db_uri.id})"
  postgres_server_fqdn = data.terraform_remote_state.persistent_dev7.outputs.postgres_server_fqdn
