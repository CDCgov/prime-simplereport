module "simple_report_api" {
  ...
  webapp_subnet_id = data.terraform_remote_state.persistent_demo.outputs.subnet_webapp_id
  lb_subnet_id     = data.terraform_remote_state.persistent_demo.outputs.subnet_lbs_id
  ...
  app_settings = {
    ...
    SPRING_DATASOURCE_SIMPLEREPORT_HIKARI_JDBCURL = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.sr_db_jdbc.id})"
