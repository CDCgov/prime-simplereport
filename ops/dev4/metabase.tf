...
module "metabase_service" {
  ...
  ai_instrumentation_key = data.terraform_remote_state.persistent_dev4.outputs.app_insights_instrumentation_key
  key_vault_id           = data.azurerm_key_vault.sr_global.id
  tenant_id              = data.azurerm_client_config.current.tenant_id

  postgres_server_name = "temp-sr-dev4-flexible-db"
  postgres_url         = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.metabase_db_uri_test.id})"
  postgres_server_fqdn = "temp-sr-dev4-flexible-db.postgres.database.azure.com"
