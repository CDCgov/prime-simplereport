...
import { ReportStreamError, ReportStreamResponse, SimpleReportReportStreamResponse } from "./rs-response";
...
export async function minimumMessagesAvailable(
    ...
    if (approxMessageCount === undefined) {
        context.log(`Queue: ${queueClient.name}. Queue message count is undefined; aborting`);
        return false;
    }
...
export async function deleteSuccessfullyParsedMessages(
    ...
    try {
        ...
        context.log(`Queue: ${queueClient.name}. The following error has occurred: ${e}`);
    }
...
export async function reportExceptions(
    ...
    const payloads: SimpleReportReportStreamResponse[] = response.warnings
        ...
        .concat(response.errors.flatMap((e) => responsesFrom(eventQueueName, context, e, true)));
    return Promise.all(
        payloads.map((p) =>
...
const responsesFrom = function (
    ...
    if (err.trackingIds) {
        ...
        context.log(
            `Queue: ${queueName}. ReportStream response ${err.scope} ${isError ? "error" : "warning"}: ${
                err.message
            }`
        );
        return [];
