module "simple_report_api" {
  ...
  app_settings = {
    SPRING_PROFILES_ACTIVE                        = "azure-dev6"
    SPRING_DATASOURCE_SIMPLEREPORT_HIKARI_JDBCURL = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.sr_dev6_db_jdbc.id})"
    ...
    DATAHUB_FHIR_API_KEY                          = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault_secret.datahub_fhir_api_key.id})"
