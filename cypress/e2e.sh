...
if [[ $RUN_OPEN = true ]]; then
  ...
  CYPRESS_CACHE_FOLDER=./tmp/Cypress yarn run cypress run --browser "$BROWSER" --spec "$SPEC_PATH" --config-file cypress.config.js --config baseUrl="$TEST_ENV$PUBLIC_URL" --env CHECK_COMMIT="$CHECK_COMMIT",CHECK_URL="$FRONTEND_URL_PATH" --record --key "$CYPRESS_RECORD_KEY"
fi;
