...
let getPatientLinkData = (results: MultiplexResult[]): VerifyV2Response => ({
...
describe("TestResult - multiple diseases", () => {
  ...
  it("should show the results for positive COVID-19 and inconclusive RSV", () => {
    const results = [
      { disease: { name: "RSV" }, testResult: "UNDETERMINED" },
      { disease: { name: "COVID-19" }, testResult: "POSITIVE" },
    ] as MultiplexResult[];
    const store = mockStore({ testResult: getPatientLinkData(results) });
    render(
    ...
    expect(within(patientApp).getAllByText("Inconclusive")).toHaveLength(1);
  });
});
