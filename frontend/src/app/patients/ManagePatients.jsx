...
const ManagePatients = () => {
  ...
  var loadFile = (file) => {
    ...
    thisReader.onloadend = function(e) {
      ...
      const jsonImport = readString(thisReader.result, { header: true, dynamicTyping: true, skipEmptyLines: "greedy" });
      ...
      jsonImport.badRows = [];
      jsonImport.errors.slice().reverse().forEach(err => {
      ...
      jsonImport.data.forEach((row, index) => {
        ...
        if (!valid) {
          ...
          err = {
            "type": "InvalidValue",
            "code": "InvalidValue",
            "message": (validate.errors[0].dataPath + validate.errors[0].keyword + validate.errors[0].message),
            "original": rem[0]
          };
          jsonImport.badRows.push(err);
          ...
          err = {
            "type": "WARNING",
            "code": "duplicateID",
            "message": (row.patientID + " already exists in local storage, updating data"),
    ...
    <main className="prime-home">
      <div className="grid-container">
        <div className="prime-container">
          ...
          <input type='file' id='uploadCSV' className='input-file' accept='.csv' onChange={csv => loadFile(csv.target.files[0])}/>
