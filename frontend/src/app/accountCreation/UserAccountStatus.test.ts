...
describe("UserAccountStatus", () => {
    ...
    it("returns set-password page for password reset", () => {
        expect(routeFromStatus(UserAccountStatus.PASSWORD_RESET)).toBe("/set-password");
    });
    it("returns set-recovery-questions page for set security questions", () => {
        expect(routeFromStatus(UserAccountStatus.SET_SECURITY_QUESTIONS)).toBe("/set-recovery-question");
    });
    ...
    it("returns sms-verify page for user pending sms verification", () => {
        expect(routeFromStatus(UserAccountStatus.SMS_PENDING_ACTIVATION)).toBe("/mfa-sms/verify");
    });
    it("returns phone-verify page for user pending call verification", () => {
        expect(routeFromStatus(UserAccountStatus.CALL_PENDING_ACTIVATION)).toBe("/mfa-phone/verify");
    });
    it("returns email-verify page for user pending email verification", () => {
        expect(routeFromStatus(UserAccountStatus.EMAIL_PENDING_ACTIVATION)).toBe("/mfa-email/verify");
    });
    it("returns google-auth-verify page for user pending google verification", () => {
        expect(routeFromStatus(UserAccountStatus.GOOGLE_PENDING_ACTIVATION)).toBe("/mfa-google-auth/verify");
    });
    it("returns okta-auth page for user pending okta verification", () => {
        expect(routeFromStatus(UserAccountStatus.OKTA_PENDING_ACTIVATION)).toBe("/mfa-okta/verify");
    });
    it("returns security-key page for user pending fido verification", () => {
        expect(routeFromStatus(UserAccountStatus.FIDO_PENDING_ACTIVATION)).toBe("/mfa-security-key");
    });
